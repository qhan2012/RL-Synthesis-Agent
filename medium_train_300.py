#!/usr/bin/env python3
"""
Medium Training Script with 300 Circuits and Exclusive Data Splits

This script uses 300 circuits with proper train/validation/evaluation splits
while maintaining ~3 hours runtime on 3080 GPU.
"""
...
class CircuitDataSplitter:
    """Handles exclusive splitting of circuit dataset into train/validation/evaluation sets."""
    
    def __init__(self, dataset, train_ratio=0.7, val_ratio=0.15, eval_ratio=0.15, random_seed=42):
        """
        Initialize data splitter with exclusive splits.
        ...
        Args:
            ...
            eval_ratio: Fraction for evaluation set (default: 0.15)
        ...
        # Create exclusive splits
        self._create_splits()
    
    def _create_splits(self):
        """Create exclusive train/validation/evaluation splits."""
        # Get all circuits and shuffle them
        all_circuits = list(self.dataset.circuits)
        random.shuffle(all_circuits)
        
        total_circuits = len(all_circuits)
        
        # Calculate split sizes
        train_size = int(total_circuits * self.train_ratio)
        val_size = int(total_circuits * self.val_ratio)
        eval_size = total_circuits - train_size - val_size  # Remainder goes to eval
        
        # Create exclusive splits
        self.train_circuits = all_circuits[:train_size]
        self.val_circuits = all_circuits[train_size:train_size + val_size]
        self.eval_circuits = all_circuits[train_size + val_size:]
        
        # Create metadata for each split
        self.train_metadata = [(circuit, self.dataset.circuit_metadata[circuit]) 
                              for circuit in self.train_circuits]
        self.val_metadata = [(circuit, self.dataset.circuit_metadata[circuit]) 
                            for circuit in self.val_circuits]
        self.eval_metadata = [(circuit, self.dataset.circuit_metadata[circuit]) 
                             for circuit in self.eval_circuits]
    ...
    def get_eval_circuits(self):
        """Get evaluation circuits with metadata."""
        return self.eval_metadata
    
    def get_split_info(self):
        """Get information about the data splits."""
        return {
            'total_circuits': len(self.dataset.circuits),
            'train_circuits': len(self.train_circuits),
            'val_circuits': len(self.val_circuits),
            'eval_circuits': len(self.eval_circuits),
            'train_ratio': len(self.train_circuits) / len(self.dataset.circuits),
            'val_ratio': len(self.val_circuits) / len(self.dataset.circuits),
            'eval_ratio': len(self.eval_circuits) / len(self.dataset.circuits)
        }
    
    def sample_training_circuits(self, num_circuits):
        """Sample circuits from training set for a batch."""
        return random.choices(self.train_metadata, k=num_circuits)


def load_medium_config_300():
    """Load medium-scale configuration for 300 circuits with ~3 hours training on 3080."""
    return {
        'algorithm': 'PPO',
        'total_timesteps': 1000000,  # INCREASED: Extended to 1M timesteps for much longer training
        'n_steps': 256,
        'batch_size': 64,  # UPDATED: Increased from 20 to 64 for faster learning
        'ppo_epochs': 4,
        'gamma': 0.99,  # INCREASED: From 0.95 to 0.99 for more consistent long-term plans
        'gae_lambda': 0.90,  # DECREASED: From 0.95 to 0.90 for faster reward response and better exploration
        'clip_range': 0.2,
        'value_loss_coef': 1.0,
        'entropy_coef': 0.01,
        'max_grad_norm': 0.5,
        'learning_rate': 5e-4,  # DECREASED: From 1e-3 to 5e-4 for more stable training
        'lr_schedule': 'linear',
        ...
        # EXPANDED: Dataset with 300 circuits and exclusive splits
        'dataset': {
            ...
            'augmentation': True,
            'caching': True,
            'max_circuits': 300,  # EXPANDED: 300 circuits for more diversity
            'train_ratio': 0.7,   # 70% training = 210 circuits
            'val_ratio': 0.15,    # 15% validation = 45 circuits
            'eval_ratio': 0.15,   # 15% evaluation = 45 circuits
            'random_seed': 42     # Reproducible splits
        },
        
        # Adjusted validation for 300 circuits
        'validation': {
            'val_interval_early': 1600,         # DOUBLED: From 800 to 1600 for half frequency
            'val_interval_late': 4000,          # DOUBLED: From 2000 to 4000 for half frequency
            'val_transition_timesteps': 40000,  # ADJUSTED: Earlier transition
            'val_episodes': 20,                 # FIXED: Increased from 10 to 20 for reduced variance
            'early_stop_patience': 999999,      # DISABLED: Set to very high value to disable early stopping
            'early_stop_min_delta': 0.005,     # DECREASED: From 0.01 to 0.005 for even less strict criteria
            'early_stop_min_timesteps': 40000, # INCREASED: From 20k to 40k timesteps before early stopping
        },
        
        # Adjusted evaluation for 300 circuits
        'eval': {
            'eval_interval': 4000,              # DOUBLED: From 2000 to 4000 for half frequency
            'eval_episodes': 45,                # FIXED: Use ALL evaluation circuits (increased from 10)
            'eval_all_circuits': True,          # FIXED: New flag to use complete evaluation set
            'metrics': ['gate_count', 'gate_reduction_percent', 'best_area', 'avg_episode_reward'],
            'test_set': 'held_out_circuits',
            'save_best_model': True,
            'online_eval': True,
            'eval_patience': 8,                 # INCREASED: From 5 to 8 for more tolerance
            'eval_min_delta': 0.05,            # DECREASED: From 0.1 to 0.05 for less strict criteria
        },
        
        # Enhanced logging
        'logging': {
            'log_interval': 400,                # ADJUSTED: Log every 400 timesteps
        ...
        'checkpoint': {
            'save_interval': 8000,              # ADJUSTED: Save every 8k timesteps
            'keep_best_n': 3,
            'save_optimizer': True
        }
    }
...
def collect_batch_with_splits(agent, env, data_splitter, batch_size, config):
    """Collect a batch of episodes using the data splitter."""
    all_observations = []
    all_actions = []
    all_rewards = []
    all_values = []
    ...
    circuits = data_splitter.sample_training_circuits(batch_size)
...
def validate_agent_with_splits(agent, env, val_circuits, num_episodes=20):
    """Validate agent performance using exclusive validation set with comprehensive metrics."""
    
    # FIXED: Use the configured number of validation episodes for better stability
    # Default increased from 5 to 10 in configuration
...
def evaluate_agent_with_splits(agent, env, eval_circuits, num_episodes=10):
    """Evaluate agent performance using exclusive evaluation set with comprehensive metrics."""
    
    # FIXED: Use all evaluation circuits for complete coverage
    use_all_circuits = True  # Always use all circuits for reliable evaluation
...
def medium_train_300():
    """Run medium training session with 300 circuits and exclusive splits."""
    print("="*70)
    print("MEDIUM TRAINING WITH 300 CIRCUITS & EXCLUSIVE SPLITS (~4 hours)")
    print("="*70)
    
    start_time = time.time()
    
    # Load configuration
    config = load_medium_config_300()
    
    # Setup logging
    logger = setup_logger('medium_training_300', level=logging.INFO)
    
    # Create dataset with 300 circuits
    try:
        ...
        max_circuits = config['dataset'].get('max_circuits', 300)
        
        if total_available < max_circuits:
            logger.warning(f"Only {total_available} circuits available, requested {max_circuits}")
            max_circuits = total_available
        
        # Limit circuits if needed
        ...
        data_splitter = CircuitDataSplitter(
            ...
            eval_ratio=config['dataset'].get('eval_ratio', 0.15),
            random_seed=config['dataset'].get('random_seed', 42)
        )
        
        # Log split information
        split_info = data_splitter.get_split_info()
        logger.info(f"Data splits - Total: {split_info['total_circuits']}, "
    ...
    val_interval_early = val_config.get('val_interval_early', 800)    # FIXED: Use correct config value
    val_interval_late = val_config.get('val_interval_late', 2000)     # FIXED: Use correct config value  
    val_transition_timesteps = val_config.get('val_transition_timesteps', 40000)  # FIXED: Use correct config value
    val_episodes = val_config.get('val_episodes', 20)                # FIXED: Use correct config value

    # Early stopping configuration
    early_stop_patience = val_config.get('early_stop_patience', 10)   # FIXED: Use correct config value
    early_stop_min_delta = val_config.get('early_stop_min_delta', 0.01)  # FIXED: Use correct config value
    early_stop_min_timesteps = val_config.get('early_stop_min_timesteps', 20000)  # FIXED: New minimum timesteps

    # Evaluation configuration
    eval_config = config.get('eval', {})
    eval_interval = eval_config.get('eval_interval', 2000)
    eval_episodes = eval_config.get('eval_episodes', 45)
    ...
    log_interval = log_config.get('log_interval', 160)  # Reduced from 400
    tb_log_interval = log_config.get('tensorboard_log_interval', 100)
    
    # Checkpoint configuration
    checkpoint_config = config.get('checkpoint', {})
    save_interval = checkpoint_config.get('save_interval', 3200)  # Reduced from 8000
    ...
    tb_log_dir = tb_dir / f'medium_train_300_{current_time}'
    ...
    logger.info("Starting medium training with 300 circuits and exclusive splits...")
    logger.info(f"Target timesteps: {total_timesteps} (adjusted for 300 circuits)")
    ...
    logger.info(f"Expected validations: ~{int(val_transition_timesteps/val_interval_early) + int((total_timesteps-val_transition_timesteps)/val_interval_late)}")
    logger.info(f"Expected evaluations: ~{int(total_timesteps/eval_interval)}")
    ...
    validation_count = 0  # FIXED: Add validation counter for debugging
    ...
    evaluation_count = 0  # FIXED: Add evaluation counter for debugging
    
    # Get exclusive circuit sets
    train_circuits = data_splitter.get_train_circuits()
    val_circuits = data_splitter.get_val_circuits()
    eval_circuits = data_splitter.get_eval_circuits()
    
    logger.info(f"Using exclusive splits: {len(train_circuits)} train, {len(val_circuits)} val, {len(eval_circuits)} eval")
    
    # FIXED: Additional logging for evaluation configuration
    ...
    try:
        with tqdm(total=total_timesteps, desc="Training Progress") as pbar:
            while timesteps < total_timesteps:
                ...
                # Collect batch of episodes using exclusive training set
                batch_data = collect_batch_with_splits(agent, env, data_splitter, batch_size, config)
                ...
                if batch_data['episode_infos']:
                    ...
                    if timesteps % 2000 == 0:  # Reduced from 5000
                ...
                if timesteps - last_validation_timestep >= current_val_interval:
                    ...
                    if val_area_reduction > best_val_performance + early_stop_min_delta:
                        ...
                        # Save best model
                        model_path = model_dir / f'best_val_model_{timesteps}.pth'
                ...
                if timesteps - last_evaluation_timestep >= eval_interval:
                    ...
                    if eval_area_reduction > best_eval_performance + eval_min_delta:
                        ...
                        model_path = model_dir / f'best_eval_model_{timesteps}.pth'
                        agent.save_model(str(model_path))
                ...
                if timesteps % save_interval == 0:
                    model_path = model_dir / f'checkpoint_{timesteps}.pth'
                    agent.save_model(str(model_path))
        ...
        final_eval_results = evaluate_agent_with_splits(
        ...
        # Save final model
        final_model_path = model_dir / f'final_model_{timesteps}.pth'
        ...
        logger.info(f"Total validations: {validation_count} (expected: ~{int(val_transition_timesteps/val_interval_early) + int((total_timesteps-val_transition_timesteps)/val_interval_late)})")
        logger.info(f"Total evaluations: {evaluation_count} (expected: ~{int(total_timesteps/eval_interval)})")
        ...
        logger.info(f"Data splits used: {split_info['train_circuits']} train, "
        ...
        if val_history:
            ...
            logger.info(f"  Total validations: {validation_count} (vs expected ~{int(val_transition_timesteps/val_interval_early) + int((total_timesteps-val_transition_timesteps)/val_interval_late)})")
        ...
        if eval_history:
            ...
            logger.info(f"  Total evaluations: {evaluation_count} (vs expected ~{int(total_timesteps/eval_interval)})")
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--help':
        print("Medium Training Script with 300 Circuits")
        print("Usage: python medium_train_300.py")
        print()
        print("This script trains an RL agent on 300 circuits with exclusive")
        print("train/validation/evaluation splits for ~4 hours on RTX 3080.")
        print()
        print("Data splits:")
        print("  - Training: 210 circuits (70%)")
        print("  - Validation: 45 circuits (15%)")
        print("  - Evaluation: 45 circuits (15%)")
        print()
        print("Enhanced configuration for longer training:")
        print("  - Extended timesteps: 120k (from 80k)")
        print("  - Increased learning rate: 5e-4 (from 3e-4)")
        print("  - Relaxed early stopping: 15 patience, 0.005 min_delta")
        print("  - Minimum 40k timesteps before early stopping")
        print("  - Exclusive circuit assignments")
        return
    
    success = medium_train_300()
    if success:
